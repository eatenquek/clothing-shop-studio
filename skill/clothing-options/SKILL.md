...
description: Use only when the user explicitly invokes $clothing-options to create or revise an A/B/C/W garment visual round.
---
...
- `generate_options` with modes `plan`, `register`, `merge`
- `record_answer`
- helper `scripts/render-options.py`

## Core references

- `references/visual-options.md` — “Workflow”, “Combining and revising”
...
- Show all four labelled visuals before asking for a choice.
- Record the selected look in production-readable words, not as a concept id alone.

## Scope boundaries

- A generated preview is never approval or a production master.
