...
The runner reads the key only from `OPENAI_API_KEY` in its environment and never reads, copies, or links a user's real Codex `auth.json`. It first tries `codex exec` with the environment key alone. If the installed Codex version does not accept that mechanism, the runner may pipe the key over stdin to `codex login --with-api-key`, writing `auth.json` only inside the throwaway `CODEX_HOME` under the throwaway `HOME`. It deletes the entire throwaway home before exiting, on success and on failure. The key is never written to the repository, a transcript, or any path outside the throwaway home, and the environment is redacted from transcripts. If `OPENAI_API_KEY` is absent, the runner exits before invoking Codex with a clear release-blocking message. Before cleanup, it copies redacted transcripts, tested tree hashes, and the selected authentication mechanism only to the explicit `--out` directory supplied by the release operator; the committed release location is `evals/green/`.
...
The six scenarios are model-backed and therefore do not run inside the deterministic unit-test gate. They are an explicit release step: commit transcripts under `evals/green/` with the exact tested core and wrapper tree hashes, and make no core or wrapper changes afterward. The deterministic suite verifies that the generated wrapper text contains the manifest's required gate tokens.
...
| Invocation is mistaken for consent or approval | Wrapper prose forbids using command text as `statement` or `user_quote`; this cannot be fully script-enforced, so recorded Codex behavioral evaluations are the release control |
...
7. All six explicit-entry scenarios have committed Codex transcripts recording the exact tested core and wrapper tree hashes, including `$` discovery, sibling-core loading, consent and approval boundaries, scope refusals, seller notices, and the one-question rule.
8. The tested core and wrapper trees are unchanged after those transcripts are recorded.
